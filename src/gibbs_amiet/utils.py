...
def extract_pressure_te(
    ...
    dx : int, optional
        Distance from the trailing edge to extract the pressure. Default is 100.
...
def get_data_info(data_file:str, verbose:bool=False):
    ...
    -------
    dict
        Dictionary containing the number of timesteps, sampling time, sampling frequency, and total time.
    '''
    with h5py.File(data_file, "r") as f:
        if verbose:
            print(f'+{"-"*20}+{"-"*20}+')
