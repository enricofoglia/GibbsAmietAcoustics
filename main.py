...
def main():
    ...
    fig.colorbar(
        c, ax=ax, orientation="vertical", fraction=0.1,
        label=r"$p-\langle p\rangle$"
    )
    ax.set_xlabel("$t$")
    ...
    xf_w, spp = ga.stats.spectrum(
        p_te, fs=fs, nperseg=nperseg, noverlap=noverlap, window=window, axis=0
    )
    ...
    plt.savefig(
        os.path.join(fig_dir, "spectrum_te.pdf"), bbox_inches="tight",
        transparent=True
    )
    ...
    ax.set_ylabel("$z/c$ [-]")
    plt.savefig(
        os.path.join(fig_dir, "coherence_funct_te.png"),
        bbox_inches="tight",
        transparent=True,
        dpi=300,
    )
    ...
    ax.set_xlim([f[1], f[-1]])
    plt.savefig(
        os.path.join(fig_dir, "coherence_length_te.pdf"),
        bbox_inches="tight",
        transparent=True,
    )
